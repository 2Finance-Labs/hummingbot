...
class TwoFinanceHummingbotSmokeTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        self.trading_pair = os.getenv("TWO_FINANCE_TRADING_PAIR", "BTC-USDT")
        self.exchange_symbol = self.trading_pair.replace("-", "/")
        self.base_asset, self.quote_asset = self.trading_pair.split("-", 1)
        self.exchange = TwoFinanceExchange(
            ...
            twofinance_state_api_url=os.getenv("TWO_FINANCE_STATE_API_URL", "http://127.0.0.1:11080"),
    ...
    async def test_connector_loads_market_data_places_and_cancels_order(self):
        ...
        self.assertEqual(self.exchange._symbol_metadata[self.trading_pair]["exchange_symbol"], self.exchange_symbol)
        ...
        self.assertGreater(self.exchange.get_balance(self.quote_asset), Decimal("0"))

        data_source = self.exchange._create_order_book_data_source()
        snapshot = await data_source._order_book_snapshot(self.trading_pair)
        self.assertEqual(snapshot.trading_pair, self.trading_pair)
